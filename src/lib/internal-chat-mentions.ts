import type { Mention, MentionTarget, MentionType } from "@/lib/internal-chat-types";
...
export function parseMentionMarkers(body: string): Array<
...
export function resolveConversationId(
  ...
  const hit =
    targets.find(
      (t) =>
        t.type === "customer" &&
        t.targetId === mention.targetId &&
        t.conversationId,
    ) ??
    targets.find(
      (t) => t.contractId === mention.targetId && t.conversationId,
    );
