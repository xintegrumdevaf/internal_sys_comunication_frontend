          ...
          <div className="p-6 overflow-y-auto flex-1 space-y-6">
            ...
            <div className="grid grid-cols-4 gap-3 text-mono text-xs">
              <div className="p-3 bg-background rounded-xl border border-border">
                <p className="text-[9px] uppercase font-bold text-muted-foreground">Total Recibos</p>
                <p className="text-base font-extrabold font-mono mt-0.5">{campaign.totalRecipients}</p>
              </div>
              <div className="p-3 bg-background rounded-xl border border-border">
                ...
                <div className="flex gap-1 text-[10px] font-mono">
                  <button
                    ...
                    className={`px-2 py-0.5 rounded border ${
                      filterStatus === "all" ? "bg-primary text-primary-foreground border-primary" : "border-border"
                    }`}
                  >
                  ...
                  <button
                    ...
                    className={`px-2 py-0.5 rounded border ${
                      filterStatus === "failed" ? "bg-danger text-white border-danger" : "border-border text-danger"
                    }`}
                  >
              ...
              <div className="border border-border rounded-xl overflow-hidden bg-background divide-y divide-border">
                {filteredRecipients.length > 0 ? (
                  filteredRecipients.map((rec, idx) => (
                    <div key={idx} className="p-3 text-xs space-y-1 hover:bg-card/50 transition-colors">
                      ...
                      {Object.keys(rec.variables || {}).length > 0 && (
                        <div className="text-[10px] font-mono text-muted-foreground flex flex-wrap gap-1 pt-0.5">
                          {Object.entries(rec.variables).map(([k, v]) => (
                            <span key={k} className="px-1.5 py-0.5 rounded bg-card border border-border">
