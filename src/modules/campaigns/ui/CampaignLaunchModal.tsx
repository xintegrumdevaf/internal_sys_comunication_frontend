  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      ...
      if (headers.length > 0) {
        ...
        const foundPhone = headers.find((h) =>
          /tel|phone|cel|movil|numero/i.test(h),
        );
              ...
              <div>
                ...
                {approvedTemplates.length === 0 && (
                  <p className="text-[11px] text-warning mt-1 flex items-center gap-1 font-semibold">
                    <AlertCircle className="size-3" /> No hay plantillas aprobadas. Crea o sincroniza una primero.
                  </p>
                )}
              </div>
              ...
              <div className="border border-border rounded-xl p-4 bg-background space-y-3">
                <div className="flex justify-between items-center">
                  <h4 className="text-xs font-extrabold uppercase tracking-widest text-muted-foreground flex items-center gap-1.5">
                    <Users className="size-3.5 text-primary" /> Destinatarios ({parsedRecipientsResult.recipients.length})
                  </h4>
                  <div className="flex bg-card p-0.5 rounded-lg border border-border text-[11px]">
                    <button
                      ...
                      className={`px-2.5 py-1 rounded-md font-bold transition-colors ${
                        inputMode === "csv" ? "bg-primary text-primary-foreground" : "text-muted-foreground"
                      }`}
                    >
                      Archivo CSV
                    </button>
                    <button
                      ...
                      className={`px-2.5 py-1 rounded-md font-bold transition-colors ${
                        inputMode === "manual" ? "bg-primary text-primary-foreground" : "text-muted-foreground"
                      }`}
                    >
                      Entrada Manual
                    </button>
                ...
                {inputMode === "csv" ? (
                  <div className="space-y-3">
                    <div className="border-2 border-dashed border-border hover:border-primary/50 rounded-xl p-4 text-center cursor-pointer bg-card/40 relative">
                      ...
                      <p className="text-[10px] text-muted-foreground">Formato: Teléfono, Nombre, Variable2...</p>
                    </div>
            ...
            <div className="md:col-span-5 space-y-4">
              ...
              <div className="p-3 bg-background border border-border rounded-xl space-y-1.5 text-xs font-mono">
                <div className="flex justify-between items-center text-muted-foreground">
                  ...
                  <span className="font-bold text-foreground">{parsedRecipientsResult.recipients.length}</span>
                </div>
        ...
        {showConfirmation && (
          <div className="p-4 bg-warning/15 border-t border-warning/30 flex items-center justify-between text-xs font-semibold animate-fade-in">
            <div className="flex items-center gap-2 text-amber-300">
              ...
              <span>
                ¿Confirmas el envío de <strong>{parsedRecipientsResult.recipients.length}</strong> mensajes de WhatsApp a través de la API de Meta Cloud?
              </span>
            </div>
