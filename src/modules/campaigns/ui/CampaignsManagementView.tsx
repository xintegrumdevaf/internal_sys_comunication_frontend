...
export function CampaignsManagementView() {
  ...
  const [selectedCampaignIdForDetails, setSelectedCampaignIdForDetails] = useState<string | null>(null);
  ...
  return (
    <AppShell title="Gestión de Campañas y Plantillas de WhatsApp" icon={Megaphone}>
      ...
      <section className="grid grid-cols-2 md:grid-cols-4 gap-4 animate-fade-up mb-5">
        <StatCard label="Enviados hoy" value={stats.enviadosHoy} hint={`${stats.totalCampanas} campañas creadas`} />
        <StatCard label="Entregabilidad" value={stats.entregabilidad} unit="%" hint="Meta ≥ 95%" tone="success" />
        <StatCard label="Fallidos" value={String(stats.fallidos)} hint="Reintentos y webhook logs" tone="warning" />
        <StatCard label="Opt-out" value={stats.optOut} unit="%" hint="Bajo la norma Meta" tone="success" />
      </section>
        ...
        <section className="grid grid-cols-12 gap-6 animate-fade-up mb-8">
          <div className="col-span-12 lg:col-span-8 space-y-4">
            {campaignsLoading ? (
              ...
              <div className="p-12 text-center bg-card border border-border rounded-xl space-y-3">
                ...
                <p className="text-xs text-muted-foreground">No hay campañas masivas registradas aún.</p>
                <button
                  type="button"
                  onClick={() => setIsCampaignModalOpen(true)}
              ...
              campaigns.map((c) => (
                ...
                >
                  <div className="flex justify-between items-start mb-3">
                    ...
                    >
                      {c.status === "in_progress" ? "En curso" : c.status === "completed" ? "Completada" : c.status}
                    </span>
                  </div>
                  ...
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    <span className="text-[10px] font-bold font-mono text-primary">{c.progress}%</span>
                  </div>
                  ...
                  <div className="grid grid-cols-3 gap-2 text-[10px] font-mono mt-3">
                    ...
                    <div className="p-2 bg-background rounded border border-border">
                      ...
                      <p className="font-bold text-xs text-emerald-400">{c.deliveredCount.toLocaleString("es-CO")}</p>
                    </div>
                    <div className="p-2 bg-background rounded border border-border">
                      <p className="text-muted-foreground text-[9px] uppercase flex items-center gap-1">
          ...
          <div className="col-span-12 lg:col-span-4">
            <div className="bg-card border border-border rounded-xl p-5 text-xs text-muted-foreground leading-relaxed space-y-3">
              ...
              <p>
                Haz clic sobre cualquier tarjeta de campaña para desplegar el monitoreo detallado en tiempo real, observar los receptores y revisar motivos de error por número telefónico.
              </p>
            </div>
          </div>
        </section>
            ...
            <div className="flex items-center gap-3">
              <div className="flex items-center gap-1 text-xs text-muted-foreground">
                ...
                <select
                  ...
                  onChange={(e) => setTemplateStatusFilter(e.target.value as TemplateStatus | "ALL")}
          ...
          <div className="bg-card border border-border rounded-xl overflow-hidden shadow-sm">
            {templatesLoading ? (
              ...
              <div className="p-12 text-center space-y-2">
                ...
                <p className="text-xs text-muted-foreground">No se encontraron plantillas con los filtros aplicados.</p>
              </div>
            ) : (
                  ...
                  <tbody className="divide-y divide-border">
                    {filteredTemplates.map((t) => (
                      <tr key={t.id} className="hover:bg-background/50 transition-colors">
                        ...
                        <td className="p-3.5 font-mono uppercase text-muted-foreground">{t.language}</td>
                        <td className="p-3.5">
        ...
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 animate-fade-in">
          <div className="bg-card border border-border rounded-2xl p-6 max-w-lg w-full space-y-4 shadow-2xl">
            <div className="flex justify-between items-center border-b border-border pb-3">
              <div>
                <span className="text-[10px] font-mono text-muted-foreground uppercase">Plantilla Meta</span>
                <h3 className="text-sm font-extrabold font-mono text-primary">{previewTemplateModal.name}</h3>
              </div>
              <button
                type="button"
