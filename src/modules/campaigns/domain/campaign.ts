...
export function buildCampaignRecipientsFromRows(
  ...
  for (const row of rows) {
    ...
    Object.keys(row).forEach((k) => {
      ...
      if (!['number', 'telefono', 'phone', 'celular', 'name', 'nombre', 'body', 'mensaje'].includes(lowerKey)) {
        variables[k] = row[k];
      }
    });
...
export function formatRoutingBehaviorSummary(config?: ChatRoutingConfig): string {
  ...
  const statusLabel =
    config.chatStatus === 'open'
      ...
      ? 'En espera'
      : 'Cerrado';
  const dept = config.departmentName && config.departmentName.trim() ? config.departmentName : 'Sin departamento';
